...
maxdistmat=[];
for frameNum=2:fileNum
    input=getFrame(inputpath,inputlist,frameNum);
    mask=getFrame(maskpath,masklist,frameNum);
    [area,center,pixellist]=blobSizeAndCenter();
    newblob.area=area;
    newblob.center=center;
    newblob.pixellist=pixellist;
    newblob.input=input;
    [feature,point]=blobFeature(newblob);
    newblob.feature=feature;
    newblob.point=point;
    newblob.group=zeros(length(newblob.feature),1);
    newblob.grouptmp=zeros(length(newblob.feature),1);
    [newblob,oldblob,pair,featuredeleteidx]=blobMatchAndUpdate(newblob,oldblob);
    
    featureGrounpUpdate(newblob,oldblob,pair);
    ...
    function [area,center,pixellist]=blobSizeAndCenter()
    ...
    function [newblob,oldblob,pair,featuredeleteidx]=blobMatchAndUpdate(newblob,oldblob)
        if(~blobInit)
            ...
            for i=1:newfeaturenum
                oldblob.feature(i)=newblob.feature(i);
                %                 oldblob.status(i)='new';
                oldblob.status(i)=1;
                oldblob.newcount(i)=1;
            ...
            grouptmpid=grouptmpid+newblobnum;
            
            
            %
            %             for i=1:newblobnum
            %                for j=1:oldblobnum
            %                    match=newblobpair(i,:)&oldblobpair(j,:);
            %                    matchmat(i,j)=sum(match);
            %                end
            %             end
    ...
    function [feature,point]=blobFeature(blob)
        gray=rgb2gray(input);
        gray(~mask)=0;
    ...
    function oldblob=tracksplit(splitgroup,newblob,oldblob,pair)
        [m,n]=size(splitgroup);
        i=1;
        j=1;
        while i<=m
            while j<=n
                if(i~=j&&splitgroup(i,j)~=0)
                    ...
                    for k=1:pairnum
                        ...
                        if(newblob.grouptmp(newfeatureid)==maxgid+mingroupid)
                            oldblob.group(oldfeatureid)=groupid;
                        end
    ...
    function oldblob=featureGrounpUpdate(newblob,oldblob,pair)
        featurenum=length(oldblob.feature);
        ...
        for i=1:pairnum
            newfeatureid=pair(i,1);
            oldfeatureid=pair(i,2);
            newgroupid=newblob.grouptmp(newfeatureid)-mingrouptmpid;
        ...
        validmatchmat=matchmat(trackarray>3,:)>3;
        ...
        oldblob=tracksplit(splitgroup,oldblob,pair);
    end
end