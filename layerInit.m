function [layer]=layerInit(frame1)
[a,b,c]=size(frame1);
...
% a learnRate
% b hit gap, such as hit=y in [x-b,x+b]

layer=struct(...
'max',zeros(a,b,c,'double'),...
'min',zeros(a,b,c,'double'),...
'lmin',zeros(a,b,'double'),...
'lmax',zeros(a,b,'double'),...
'lmean',zeros(a,b,'double'),...
'lminc',zeros(a,b,'uint32'),...
'lmaxc',zeros(a,b,'uint32'),...
'gap',ones(a,b,c,'double')*5,...
'mean',zeros(a,b,c,'double'),...
'smean',zeros(a,b,c,'double'),...
'hmean',zeros(a,b,c,'double'),...
...
if(c==1)
    ...
    layer.hmean=double(frame1);
    layer.smean=double(frame1);
    layer.lmax=sqrt(sum(double(frame1).^2,3));
    layer.lmin=layer.lmax;
    layer.lmean=layer.lmax;
